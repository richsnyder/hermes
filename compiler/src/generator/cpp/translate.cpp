...
translate(std::shared_ptr<state::datatype> a_datatype)
{
  if (a_datatype->is_alias())
  {
    ...
    return std::make_shared<alias>(as_alias->name(), root);
  }
  else if (a_datatype->is_enumeration())
  {
    return std::make_shared<enumeration>(a_datatype->name());
  }
  else if (a_datatype->is_exception())
  {
    ...
    return std::make_shared<exception>(as_exception->name(), fields);
  }
  else if (a_datatype->is_structure())
  {
    ...
    return std::make_shared<structure>(as_structure->name(), fields);
  }
